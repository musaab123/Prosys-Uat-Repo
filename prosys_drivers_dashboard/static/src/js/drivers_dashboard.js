...
odoo.define('prosys_drivers_dashboard.Dashboarddrivers', function (require) {
"use strict";
console.log("started");
...
var DriversDashboard = AbstractAction.extend({
    template: 'DriversDashboardMain',
    cssLibs: [
    ...
    jsLibs: [
     ...
     events: {
             ...
             'click .assigned_deliveries':'assigned_deliveries',
             'click .unassigned_deliveries':'unassigned_deliveries',
             'click .in_transit_deliveries':'in_transit_deliveries',
             'click .delivered_deliveries':'delivered_deliveries',
             'click .delivered_return_deliveries':'delivered_return_deliveries',
             'click .cancelled_deliveries':'cancelled_deliveries',
    ...
    init: function(parent, context) {
        ...
        this.date_to = moment();
        this.dashboards_templates = ['LoginDriverDetails'];
        this.employee_birthday = [];
        this.upcoming_events = [];
        this.announcements = [];
        this.login_employee = [];
        this.delivery_orders_counts = [];
        // this.driver_unassgined_order_counts = [];
    ...
    //         var def1 =  self._rpc({
    //             model: 'stock.picking',
    //             method: 'get_unassigned_drivers_counts'
    //         }).then(function(result) {
    //             self.driver_unassgined_order_counts =  result[0];
    //         });
    //     return $.when(def1);
    //     });
    // },
    ...
    willStart: function(){
        ...
        .then(function() {
            var def1 =  self._rpc({
                model: 'stock.picking',
                method: 'get_drivers_deliveries_counts'
            }).then(function(result) {
                self.delivery_orders_counts =  result[0];
            });
        return $.when(def1);
        });
    },
    ...
    start: function() {
            ...
            var self = this;
            this.set("title", 'Drivers Dashboard');
    ...
    render_dashboards: function() {
        var self = this;
        var templates = []
        templates = ['LoginDriverDetails'];
        _.each(templates, function(template) {
            self.$('.o_drivers_dashboard').append(QWeb.render(template, {widget: self}));
        });
    },
    ...
    //     //     // self.$('.o_drivers_dashboard').empty();
    //     //     // self.render_dashboards();
    //     //     // self.render_graphs();
    //     // });
    // },
...
//         },
        ...
        // new function for unassgin order
        unassigned_deliveries: function(ev){
            var self = this;
            ev.stopPropagation();
            ev.preventDefault();
            ...
            this.do_action({
                name: _t("Assigned Delivery Orders"),
                type: 'ir.actions.act_window',
                res_model: 'stock.picking',
                view_mode: 'tree,form',
                views: [[false, 'list'],[false, 'form']],
                domain: [['driver_id','=', false],['driver_status','!=', 'Assigned'],['state','=', 'done'],['picking_type_id.code', '=', 'outgoing'], ['origin', 'not ilike', 'Return']],
                target: 'current' //self on some of them
            }, {
                    on_reverse_breadcrumb: this.on_reverse_breadcrumb
            });
        ...
        assigned_deliveries: function(ev){
            var self = this;
            ev.stopPropagation();
            ev.preventDefault();
            ...
            this.do_action({
                ...
                view_mode: 'tree,form',
                views: [[false, 'list'],[false, 'form']],
                domain: [['driver_id','!=', false],['driver_status','=', 'Assigned'],['picking_type_id.code', '=', 'outgoing'], ['origin', 'not ilike', 'Return']],
                target: 'current' //self on some of them
            }, {
                    on_reverse_breadcrumb: this.on_reverse_breadcrumb
            });
        },
        in_transit_deliveries: function(ev){
            var self = this;
            ev.stopPropagation();
            ev.preventDefault();
            ...
            this.do_action({
                name: _t("In-Transit Delivery Orders"),
                type: 'ir.actions.act_window',
                res_model: 'stock.picking',
                view_mode: 'tree,form',
                views: [[false, 'list'],[false, 'form']],
                domain: [['driver_id','!=', false],['driver_status','=', 'In-Transit'],['picking_type_id.code', '=', 'outgoing'], ['origin', 'not ilike', 'Return']],
                target: 'current' //self on some of them
            }, {
                    on_reverse_breadcrumb: this.on_reverse_breadcrumb
            });
        ...
        delivered_deliveries: function(ev){
            var self = this;
            ev.stopPropagation();
            ev.preventDefault();
            ...
            this.do_action({
                name: _t("Delivered Delivery Orders"),
                type: 'ir.actions.act_window',
                res_model: 'stock.picking',
                view_mode: 'tree,form',
                views: [[false, 'list'],[false, 'form']],
                domain: [['driver_id','!=', false],['driver_status','=', 'Delivered'],['picking_type_id.code', '=', 'outgoing'], ['origin', 'not ilike', 'Return']],
                target: 'current' //self on some of them
            }, {
                    on_reverse_breadcrumb: this.on_reverse_breadcrumb
            });
        ...
        delivered_return_deliveries: function(ev){
            var self = this;
            ev.stopPropagation();
            ev.preventDefault();
            ...
            this.do_action({
                name: _t("Delivered with Return Delivery Orders"),
                type: 'ir.actions.act_window',
                res_model: 'stock.picking',
                view_mode: 'tree,form',
                views: [[false, 'list'],[false, 'form']],
                domain: [['driver_id','!=', false],['driver_status','=', 'Delivered with Return'],['picking_type_id.code', '=', 'incoming'],['origin', 'ilike', 'Return']],
                target: 'current' //self on some of them
            }, {
                    on_reverse_breadcrumb: this.on_reverse_breadcrumb
            });
        ...
        cancelled_deliveries: function(ev){
            var self = this;
            ev.stopPropagation();
            ev.preventDefault();
            ...
            this.do_action({
                name: _t("Cancelled Delivery Orders"),
                type: 'ir.actions.act_window',
                res_model: 'stock.picking',
                view_mode: 'tree,form',
                views: [[false, 'list'],[false, 'form']],
                domain: [['driver_id','!=', false],['driver_status','=', 'Cancelled'],['picking_type_id.code', '=', 'outgoing'], ['origin', 'not ilike', 'Return']],
                target: 'current' //self on some of them
            }, {
                    on_reverse_breadcrumb: this.on_reverse_breadcrumb
            });
...
//                 }
    ...
    core.action_registry.add('drivers_dashboard', DriversDashboard);
