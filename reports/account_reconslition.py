...
class BiReportPartnerLedger(models.AbstractModel):
    _name = 'report.bi_partner_ledger_report.partner_balance_manching_custom'
    _description="Report Partner Ledger"

    def _lines(self, data, partner):
        _logger.info("at _lines")
        _logger.info("data")
        _logger.info(data)
        full_account = []
    ...
    def _calculate_initial_balancne(self,data,partner,final_row):
        _logger.info("at calculate_initial_balance")
        domain = []
        ctx = self.env.context
        if ctx.get('used_context'):
