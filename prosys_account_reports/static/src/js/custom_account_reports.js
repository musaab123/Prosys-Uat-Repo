odoo.define('prosys_account_reports.account_report_generic', function (require) {
    'use strict';
...
var M2MFilters = Widget.extend(StandaloneFieldManagerMixin, {
    ...
    var M2MSalesTeamFilters = Widget.extend(StandaloneFieldManagerMixin, {
    /**
     * @constructor
     * @param {Object} fields
     */
    ...
    accountReportsWidget.include({
        ...
        custom_events: _.extend({}, accountReportsWidget.prototype.custom_events, {
            ...
            'value_changed': function (ev) {
                ...
                self.report_options.salesteams = ev.data.team_ids;
                console.log('ev.data.team_ids========',ev.data.team_ids)
        ...
        render_searchview_buttons: function () {
            ...
            self._super();
            if (this.report_options.salesteam) {
                if (!this.M2MSalesTeamFilters) {
                    var fields = {};
                    if (this.report_options.salesteams) {
                        fields['team_ids'] = {
                            label: _t('SalesTeam'),
                            modelName: 'crm.team',
                            value: this.report_options.salesteams.map(Number),
                        };
                    }

                    if (!_.isEmpty(fields)) {
                        this.M2MSalesTeamFilters = new M2MSalesTeamFilters(this, fields,'value_changed');
                        this.M2MSalesTeamFilters.appendTo(this.$searchview_buttons.find('.js_sales_team_m2m'));
                    }
                } else {
                    this.$searchview_buttons.find('.js_sales_team_m2m').append(this.M2MSalesTeamFilters.$el);
                }
            }
        },
