...
class Accounting_reportPartner_ledger(models.TransientModel):
    _name = "accounting_report.partner_ledger_recon"
    ...
    partner_custom = fields.Many2one('res.partner', string='Partner')
    ...
    def print_partner_ledger_reconslition(self):
        data = {}
        data['computed'] = {}
        ...
        if self.partner_custom:
            partner_ids = [self.partner_custom.id]
        else:
        ...
        if self._context.get('report_type') != 'excel':
            return self.env.ref('bi_partner_ledger_report.partner_balance_match_pdf_report_action').with_context(
            ...
            for partner in docs:
                ...
                debit = self.env['report.bi_partner_ledger_report.partner_balance_manching_custom'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(final_dict['data'], partner,
                                                                                        'debit')
                credit = self.env['report.bi_partner_ledger_report.partner_balance_manching_custom'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(final_dict['data'], partner,
                                                                                        'credit')
                balance = self.env['report.bi_partner_ledger_report.partner_balance_manching_custom'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(final_dict['data'], partner,
                                                                                        'debit - credit')
                ...
                if self.initial_balance :
                    final_data=self.env['report.bi_partner_ledger_report.partner_balance_manching_custom'].with_context(
                    ...
                    if final_data: 
                        for line in final_data:
                            ...
                            row +=1
                for line in self.env['report.bi_partner_ledger_report.partner_balance_manching_custom'].with_context(
