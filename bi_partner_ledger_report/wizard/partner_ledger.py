...
class Accounting_reportPartner_ledger(models.TransientModel):
    ...
    def print_partner_ledger(self):
        ...
        for partner_id in partner_ids:
            ...
            # if self.partner_custom:
            #     if partner_id == self.partner_custom.id:
            #         domain = [('partner_id', '=', partner_id), ('company_id', '=', self.env.company.id)]
            #     else:
            #         continue
            # else:
            ...
            team_name = account_move_lines and account_move_lines[0].team_id.name or ''
        ...
        if self._context.get('report_type') != 'excel':
            return self.env.ref('bi_partner_ledger_report.partner_ledger_pdf_report_action').with_context(
            ...
            for partner in docs:
                ...
                debit = self.env['report.bi_partner_ledger_report.partner_ledger_pdf_report'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(final_dict['data'], partner,
                                                                                        'debit')
                credit = self.env['report.bi_partner_ledger_report.partner_ledger_pdf_report'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(final_dict['data'], partner,
                                                                                        'credit')
                balance = self.env['report.bi_partner_ledger_report.partner_ledger_pdf_report'].with_context(
                    reconciled=self.reconciled, used_context=used_context)._sum_partner(final_dict['data'], partner,
                                                                                        'debit - credit')
                ...
                if self.initial_balance :
                    final_data=self.env['report.bi_partner_ledger_report.partner_ledger_pdf_report'].with_context(
                    ...
                    if final_data: 
                        for line in final_data:
                            ...
                            row +=1
                for line in self.env['report.bi_partner_ledger_report.partner_ledger_pdf_report'].with_context(
